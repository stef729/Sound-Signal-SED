...
class DataGenerator(object):
    ...
    def __init__(self, features_dir, scalar, batch_size, holdout_fold, seed=1234):
        ...
        self.feature_names = sorted(os.listdir(features_dir))
        ...
        self.train_num = math.ceil(0.7 *len(self.feature_names))
        self.train_feature_names = random.sample(self.feature_names, self.train_num)
        for self.validate_feature_name in self.feature_names:
            if self.validate_feature_name not in self.train_feature_names:
                self.validate_feature_names.append(self.validate_feature_name)
        ...
        for feature_name in self.train_feature_names:
            ...
            (feature, event_matrix) = self.load_hdf5(feature_path)
    ...
    def generate_train(self):
        ...
        while True:
            ...
            # if batch_feature.shape[0] < 2:
            #     batch_event_matrix = batch_event_matrix.reshape(batch_feature.shape[0], batch_event_matrix.shape[0],
            #                                                     batch_event_matrix.shape[1], batch_event_matrix.shape[2])
